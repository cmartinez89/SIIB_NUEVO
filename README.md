...
**Fase 5 (Reportes) — primer lote real, 2026-08-12:** 4 reportes con exportación a Excel (`exceljs`) — Compras (requisiciones vs. órdenes), Almacén (movimientos + resumen), Báscula (bitácora), Leche (por cliente). El original tiene ~40 endpoints de reporte repartidos en todos los módulos; portarlos de golpe sería el "big-bang" que este roadmap evita a propósito — el resto se agrega junto con la profundización de cada módulo fuente (Nómina, Alimentación, etc.), no como proyecto aparte.
